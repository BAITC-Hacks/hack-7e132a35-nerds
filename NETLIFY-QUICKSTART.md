...
После этого откройте **Connect** в панели Supabase и выберите **Transaction pooler**. Скопируйте строку подключения: используйте адрес и порт `6543`, базу `postgres` и имя пользователя формата `akim_app.ВАШ_PROJECT_REF`. Вставьте созданный пароль вместо пароля в строке. Строка должна использовать SSL (`sslmode=require`). Не используйте `localhost`, `127.0.0.1`, прямой адрес `db.…:5432` или Docker-имя `db`: они не подходят для Netlify Functions. Если пароль содержит символы кроме букв и цифр, их нужно URL-кодировать; проще сгенерировать 32 буквенно-цифровых символа.
...
- Base directory: пустая (корень репозитория)
...
- `{"status":"ok"}` — Netlify подключился к базе.
